...
# Create the agent. We use cache_resource to ensure the agent is only created once and reused across multiple calls.
@st.cache_resource
def get_agent():
    agent = create_agent(
        model=llm,
...
st.subheader("Task Bot - Manage your Tasks")
...
if prompt:
    ...
    st.markdown(result)
    st.session_state.messages.append({"role": "ai", "content": result})  # Store AI response in session state